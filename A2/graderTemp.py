...
def extractPath(startLocation: str, search: util.SearchAlgorithm) -> List[str]:
    ...
    """
    print("search.actions >>>> ", search.actions)
...
class Test_4c(GradedTestCase):
    ...
    def t_4c_aStar(
        ...
        baseProblem = submission.WaypointsShortestPathProblem(
            startLocation, waypointTags, endTag, cityMap
        )
        aStarProblem = submission.aStarReduction(baseProblem, heuristic)
    ...
    def test_2(self):
        ...
        self.t_4c_aStar(
            ...
            heuristic=submission.NoWaypointsHeuristic(endTag, cityMap),
            cityMap=cityMap,
            expectedCost=8,
        )
    # @graded(timeout=2)
    # def test_2(self):
    #     """4c-2-basic1: basic no waypoints heuristic A* on Stanford map (4c-astar-1)."""
