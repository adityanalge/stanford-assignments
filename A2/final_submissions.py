...
class ShortestPathProblem(SearchProblem):
    ...
    def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
        ...
        for adjacentLocation, distance in self.cityMap.distances[state.location].items():
            successors.append((adjacentLocation, State(adjacentLocation, None), distance))
        return successors
...
def getStanfordShortestPathProblem() -> ShortestPathProblem:
    ...
    # ### START CODE HERE ###
    startLandmark = "gates"
    endLandmark = "memorial_church"

    startLocationTag = f"landmark={startLandmark}"
    endLocationTag = f"landmark={endLandmark}"

    # Find start and end locations based on tags
    startLocation = locationFromTag(startLocationTag, cityMap)
    endTag = locationFromTag(endLocationTag, cityMap)

    if startLocation is None or endTag is None:
        raise ValueError("Start location or end location not found in the map.")
